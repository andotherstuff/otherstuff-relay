...
class EventBuffer {
  ...
  constructor(
    ...
    private insertBatchFn: (events: NostrEvent[]) => Promise<void>,
  ) {}
  ...
  addEvent(event: NostrEvent): Promise<void> {
    return new Promise((resolve) => {
      ...
      if (this.buffer.length >= this.config.batching.batchSize) {
  ...
  private async flush(): Promise<void> {
    ...
    try {
      await this.insertBatchFn(eventsToProcess);
...
export class ClickHouseClient {
  ...
  constructor(private config: Config) {
    ...
    this.eventBuffer = new EventBuffer(
      config,
      (events) => this.insertBatch(events),
    );
  }
  ...
  private async makeRequest(
    ...
    url.searchParams.set("database", this.config.clickhouse.database);
    ...
    if (this.config.clickhouse.user) {
      const auth = this.config.clickhouse.password
        ? `${this.config.clickhouse.user}:${this.config.clickhouse.password}`
        : this.config.clickhouse.user;
  ...
  private async insertBatch(events: NostrEvent[]): Promise<void> {
    ...
    await this.makeRequest(query, eventData);
  }
  ...
  async initDatabase(): Promise<void> {
    try {
      ...
      await this.makeRequest(
        `CREATE DATABASE IF NOT EXISTS ${this.config.clickhouse.database}`,
      );
  ...
  async insertEvent(event: NostrEvent): Promise<boolean> {
    try {
  ...
  async queryEvents(filter: NostrFilter): Promise<NostrEvent[]> {
    try {
      ...
      if (filter.ids?.length) {
        ...
        const escapedIds = limitedIds.map((id) =>
          `'${id.replace(/'/g, "\\'")}'`
        )
          .join(",");
